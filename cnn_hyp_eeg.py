...
train_step = tf.train.AdamOptimizer(1e-2).minimize(euc_distance)
...
def forward_batch(n, sess, eegs, hyps, ages):
    ...
    hyps = hyps[r]
    ages = ages[r]
    feed = {spec_ph: eegs[:n], ages_ph:ages[:n], hyp_ph:hyps[:n], keep_prob:0.5}
    agest, dist, _ = sess.run((out_t, euc_distance, train_step), feed_dict=feed)
    print(dist)
...
for k in range(500000):
    ...
    if k % 100 == 0:
        feed = {spec_ph: eval_stft, hyp_ph:e_hyp_padded, ages_ph:eval_labels, keep_prob:1}
        agest, dist = sess.run((out_t, mape_dist), feed_dict = feed)

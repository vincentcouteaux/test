...
def scatter_images(images, phi, psis, orders=[2], sub_factor=8, js=None):
    out = []
    c = 0
    for im in images:
        if c%100==0:
...
def scatter_color_images(images, phi, psis, orders=[2], sub_factor=8, js=None):
    images_red = images[:, :, :, 0]
    images_green = images[:, :, :, 1]
    images_blue = images[:, :, :, 2]
    print("Scattering red")
    scat_red = scatter_images(images_red, phi, psis, orders, sub_factor, js)
    print("Scattering green")
    scat_green = scatter_images(images_green, phi, psis, orders, sub_factor, js)
    print("Scattering blue")
    scat_blue = scatter_images(images_blue, phi, psis, orders, sub_factor, js)
    print("concatenating...")
